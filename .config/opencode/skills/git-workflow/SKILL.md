...
ISSUE_NUMBER=$(echo "$BASE_BRANCH" | grep -oP '^\d+')
```

Record `BASE_BRANCH` and `ISSUE_NUMBER` (may be empty if branch has no issue number prefix). All worktrees and branches will be named relative to these.
...
issue_number: <ISSUE_NUMBER>  (omit if empty)
```
